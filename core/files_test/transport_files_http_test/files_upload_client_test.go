...
/*
  示例：模拟【客户端】(同postman/浏览器)上传文件,且支持上传多文件 add by syf 2020.5.8
*/

//测试模拟客户端上传文件，等价于postman或浏览器功能
func TestClientUploadFiles(t *testing.T) {
	...
	//本示例模拟发送单个文件
	filename1 := "/tmp/syf/rocksdb.zip"
	sendFileNames = append(sendFileNames, filename1)
	if err := sendMultiFiles(requestParams, sendFileNames, requestUrl); err != nil {
		logs.Error("send file fail,", err)
		return
	}
}
...
func sendMultiFiles(params map[string]string, filesname []string, url string) error {
	...
	for _, filename := range filesname {
		...
		if err != nil {
			logs.Error("bodyWriter.CreateFormFile err:", err)
			return err
		}
		//打开文件
		fd, err := os.Open(filename)
		if err != nil {
			logs.Error("os.Open filename err:", err)
			return err
		}
		defer fd.Close()
		//把第一个文件流写入到缓冲区里去
	...
	//创建一个http客户端请求对象
	client := &http.Client{}
	logs.Info("send files to url:", url)
	//创建一个post请求
	...
	if err != nil {
		logs.Error("send jdchaintools request fail,err:", err.Error())
		return err
	}
	defer data.Body.Close()
	logs.Info("jdchaintools response:", string(bytes))
	//解析响应结果
	logs.Info("dial url:%s get response:%s", url, string(bytes))
	return nil
}
